...
const App = () => { 
  ...
  async function getEvolutions (id){
   ...
   const data= await response.json()
    ...
    let pokemonEvolArray = []

   let pokemonLv1 = (data.chain.species.name)
   let pokemonLv1Img =await getPokemonImgs(pokemonLv1)
   pokemonEvolArray.push([pokemonLv1,pokemonLv1Img])

    if(data.chain.evolves_to.length !==0){
        let pokemonLv2 = data.chain.evolves_to[0].species.name
        let pokemonLv2Img = await getPokemonImgs(pokemonLv2)
        pokemonEvolArray.push([pokemonLv2, pokemonLv2Img])
        

        if(data.chain.evolves_to[0].evolves_to.length !==0){
            let pokemonLv3 = data.chain.evolves_to[0].evolves_to[0].species.name
            let pokemonLv3Img = await getPokemonImgs(pokemonLv3)
            pokemonEvolArray.push([pokemonLv3, pokemonLv3Img])
            
        }
    }
    setPokemonEvolutions(pokemonEvolArray)
  }
